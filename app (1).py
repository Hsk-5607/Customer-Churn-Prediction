...
# ── Label mappings (must stay fixed — same order as training) ─────────────────
...
@st.cache_resource(show_spinner="Training model on customer data…")
def train_model():
    ...
    for col, mapping in [
        ...
        ("BookedHotelOrNot",         HOTEL_BOOKING),
    ]:
...
with st.sidebar:
    ...
    st.divider()
    st.info("Predicts whether a travel customer will **churn** based on demographic and behavioral attributes using Random Forest.")
...
if st.button("🔍 Predict Churn Risk", use_container_width=True):
    ...
    with r1:
        if prediction == 1:
            st.markdown('<div class="result-box churn">⚠️ HIGH CHURN RISK<br><span style="font-size:15px">This customer is likely to churn</span></div>', unsafe_allow_html=True)
        else:
            st.markdown('<div class="result-box no-churn">✅ LOW CHURN RISK<br><span style="font-size:15px">This customer is likely to stay</span></div>', unsafe_allow_html=True)
    ...
    with r2:
        ...
        ax.pie([safe_prob, churn_prob], colors=["#0D9488", "#DC2626"],
    ...
    st.dataframe(pd.DataFrame({
        "Feature": ["Age","Frequent Flyer","Annual Income Class","Services Opted","Social Media Sync","Hotel Booked"],
        "Value":   [age, frequent_flyer, annual_income, services_opted, social_media, hotel_booked]
    }), use_container_width=True, hide_index=True)

    st.divider()
    st.markdown("### 💡 Business Recommendation")
    if prediction == 1:
        st.warning("**Action Required:** Consider offering a **personalized discount**, **loyalty rewards**, or a **dedicated support call** to retain this customer.")
    else:
        st.success("**Customer is engaged!** Keep nurturing with **exclusive travel deals** and **loyalty program updates**.")

# ── Feature Importance ────────────────────────────────────────────────────────
st.divider()
st.markdown("## 📈 Feature Importance")
feat_names  = ["Age","FrequentFlyer","AnnualIncomeClass","ServicesOpted","AccountSyncedToSocialMedia","BookedHotelOrNot"]
importances = model.feature_importances_
sorted_idx  = np.argsort(importances)

fig2, ax2 = plt.subplots(figsize=(9, 4))
colors = ["#0D9488" if i == sorted_idx[-1] else "#5EEAD4" for i in sorted_idx]
bars   = ax2.barh([feat_names[i] for i in sorted_idx], importances[sorted_idx],
...
# ── Footer ────────────────────────────────────────────────────────────────────
st.divider()
st.markdown("<div style='text-align:center;color:#64748B;font-size:13px'>✈️ Customer Churn Predictor | B.Tech Gen AI Final Project | Rishabh Bheda | Built with Streamlit + scikit-learn</div>", unsafe_allow_html=True)
